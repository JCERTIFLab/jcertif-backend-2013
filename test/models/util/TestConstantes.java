package models.util;

/**
 * @author MSOMDA
 *
 */
public interface TestConstantes {

	public static final String COLLECTION_CATEGORY = "category";
	public static final String COLLECTION_PARTICIPANT = "participant";
	public static final String COLLECTION_SPEAKER = "speaker";
	public static final String COLLECTION_SPONSOR = "sponsor";
	public static final String COLLECTION_SPONSOR_LEVEL = "sponsor_level";
	public static final String COLLECTION_SESSION = "session";
	public static final String COLLECTION_SESSION_STATUS = "session_status";
	public static final String COLLECTION_TITLE = "title";
	public static final String COLLECTION_SITE = "site";
	public static final String COLLECTION_ROOM = "room";
}
